...
def _unflatten_optim_state_dict(
    ...
    for param_group in optim.param_groups:
        # print("param_group.keys()", param_group.keys())
        ...
        for param in param_group[_PARAMS]:
            ...
            for fqn in info.fqn_param_mapping[param]:
                ...
                for state_name in optim.state[param].keys():
                    # print(f"key: {_STATE}.{fqn}.{state_name}")
                    try:
                        cast(DictValueType, state[fqn])[state_name] = state_dict[
                            f"{_STATE}.{fqn}.{state_name}"
                        ]
                    except KeyError:
                        print(f"KeyError: {_STATE}.{fqn}.{state_name}")
                        if "lm_head" in fqn:
                            fqn_ = fqn.replace("lm_head", "model.embeddings")
                            print(f"  trying with: {_STATE}.{fqn_}.{state_name}")
                            x = state_dict[f"{_STATE}.{fqn_}.{state_name}"]
                            print(x.shape)
                            x = x.T if x.dim() == 2 else x
                            cast(DictValueType, state[fqn])[state_name] = x
                            print(x.shape)
...
def _load_optim_state_dict(
    ...
    for optim in optimizers:
        ...
        optim_state_dict['param_groups'] = [
            {
                k: (v if k != "params" else [e for e in v if "lm_head.weight" not in e])
                for k, v in x.items()
            }
            for x in optim_state_dict['param_groups']
        ]
