#!/bin/bash
#SBATCH --job-name=hgrn_s90_dyt_quantized_train
...
## Notes:
# - This script is based on train_hgrn_s90_dyt.sh but includes options for quantization-aware training
...
# - The model is first processed with dynamic_tanh_norm.py post-initialization
...
# Create a wrapper script for training that applies DyT conversion and quantization
cat > train_with_dyt_quant.py << 'EOF'
import os
import sys
import torch
from transformers import AutoConfig, AutoModelForCausalLM

# Add the local path to the python path to import our custom modules
...
# Run the training with both DyT normalization and quantization-aware training
NNODE=1 NGPU=4 LOG_RANK=0 bash train.sh \
  --job.config_file train.toml \
  --job.dump_folder /export/work/sabreu/flame/exp/hgrn-sparse-340M-10B/batch${batchsize}.gpu4.sparse90.steps20480.lr3e-4.dyt.quantized_${weight_format}_${activation_format} \
